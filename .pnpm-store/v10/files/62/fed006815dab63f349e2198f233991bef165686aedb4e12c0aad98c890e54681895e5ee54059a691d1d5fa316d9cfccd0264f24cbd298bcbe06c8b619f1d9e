...
describe("DeferredReadableStream", () => {
  it("should create a readable stream that can be read after setting source", async () => {
    const deferred = new DeferredReadableStream();
  ...
  it("should handle multiple concurrent read operations", async () => {
    ...
    expect((_a = results[0]) == null ? void 0 : _a.value).toBe(1);
    expect((_b = results[1]) == null ? void 0 : _b.value).toBe(2);
    expect((_c = results[2]) == null ? void 0 : _c.value).toBe(3);
  ...
  it("source can be set by another deferred stream after calling detach", async () => {
    ...
    expect(result2.value).toBeUndefined();
    reader.releaseLock();
    const deferred2 = new DeferredReadableStream();
  ...
  it("should transfer source between deferred streams while reading is ongoing", async () => {
    const deferred1 = new DeferredReadableStream();
    const deferred2 = new DeferredReadableStream();
    const source = new ReadableStream({
      async start(controller) {
        for (let i = 0; i < 4; i++) {
...
//# sourceMappingURL=deferred_stream.test.js.map