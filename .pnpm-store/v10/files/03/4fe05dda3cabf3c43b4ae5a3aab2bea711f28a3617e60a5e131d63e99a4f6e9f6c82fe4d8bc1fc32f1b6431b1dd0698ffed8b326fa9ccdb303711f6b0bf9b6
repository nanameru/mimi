...
async function getWebWorkerCode() {
  const sha256Module = await import("./sha256-wrapper-GDBS2T5O.mjs");
...
async function* sha256(buffer, opts) {
  ...
  if (isFrontend) {
    ...
    if (!wasmModule) {
      wasmModule = await import("./sha256-wrapper-GDBS2T5O.mjs");
    }
...
var _XetBlob = class extends Blob {
  constructor(params) {
    ...
    __privateAdd(this, _loadReconstructionInfo);
    ...
    __privateAdd(this, _reconstructionInfoPromise, void 0);
  ...
  slice(start = 0, end = this.size) {
    ...
    const slice = __privateMethod(this, _clone, clone_fn).call(this);
  ...
  async arrayBuffer() {
    const result = await __privateMethod(this, _fetch, fetch_fn).call(this);
  ...
  stream() {
    const stream = new TransformStream();
    __privateMethod(this, _fetch, fetch_fn).call(this).then((response) => response.pipeThrough(stream)).catch((error) => stream.writable.abort(error.message));
    return stream.readable;
  }
};
...
loadReconstructionInfo_fn = function() {
  ...
  __privateSet(this, _reconstructionInfoPromise, (async () => {
    ...
    const resp = await this.fetch((_a = this.reconstructionUrl) != null ? _a : `${connParams.casUrl}/v1/reconstruction/${this.hash}`, {
      headers: {
...
fetch_fn = async function() {
  if (!this.reconstructionInfo) {
    await __privateMethod(this, _loadReconstructionInfo, loadReconstructionInfo_fn).call(this);
  }
  ...
  const iterator = readData(
    ...
    this.fetch,
    this.end - this.start,
    __privateMethod(this, _loadReconstructionInfo, loadReconstructionInfo_fn).bind(this)
  );
...
function computeNumOfParamsByDtypeSingleFile(header, quantConfig) {
  ...
  for (const [tensorName, v] of typedEntries(tensors)) {
    ...
    counter[v.dtype] = ((_a = counter[v.dtype]) != null ? _a : 0) + elements * multiplier;
  }
  return counter;
}
